...
3. Runs a batch of cross-session aggregate queries (load volume, Core Web Vitals, TTFB phases, resources, third-party domains, long tasks, errors, browser/device — TTFB phases feed the slow-TTFB finding only, they have no panel of their own) plus the instance-scoped queries for that one representative session
...
| File | Contents |
|---|---|
| `full-page-analysis-{page}-{date}.html` | Standalone interactive report: page experience metrics, resource waterfall, findings, analyst notes, slowest/heaviest resources, third-party domains, long tasks and errors, and a browser/device breakdown |
...
- **Page experience metrics** — p50/p75/p95 LCP, FCP, INP, CLS and TTFB across all loads, one tile per metric against its own thresholds
